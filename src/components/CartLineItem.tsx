...
const CartLineItem = ({ item, dispatch, REDUCER_ACTION }: PropsType) => {
  ...
  const content = (
    <li className='cart__item'>
      ...
      <div arial-label='Item Name'>{item.name}</div>
      <div arial-label='Price per Item'>
      ...
      <select
        ...
        value={item.qty}
        aria-lable='Item Quantity'
        onChange={onChangeQty}>
        {options}
      </select>
      <div className='cart__item-subtotal' aria-lable='Line Item Subtotal'>
        {new Intl.NumberFormat('en-US', {
          style: 'currency',
          currency: 'USD',
        }).format(lineTotal)}
      ...
      <button
        ...
        aria-lable='Remove Item From Cart'
